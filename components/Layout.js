...
function Icon({ name, size = 18 }) {
  ...
  switch (name) {
    case 'dashboard': return <svg {...common}><path d="M4 13h8V4H4v9Zm0 7h8v-5H4v5Zm10 0h6V11h-6v9Zm0-18v7h6V2h-6Z" stroke="currentColor" strokeWidth="1.8" /></svg>;
    case 'wrench': return <svg {...common}><path d="M20 7a6 6 0 0 1-8.9 5.2L7.3 16l.7.7-1.6 1.6-2.8-2.8 1.6-1.6.7.7 3.8-3.8A6 6 0 1 1 20 7Z" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /></svg>;
    case 'chart': return <svg {...common}><path d="M4 19V5M8 17v-6M12 17V7M16 17v-9M20 17v-4M4 19h18" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" /></svg>;
    case 'shield': return <svg {...common}><path d="M12 2 20 6v6c0 5-3.4 9.4-8 10-4.6-.6-8-5-8-10V6l8-4Z" stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round" /><path d="M9 12l2 2 4-5" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /></svg>;
    default: return null;
  }
}
...
export default function Layout({
  ...
  const displayName = useMemo(() => {
    if (!user) return '';
    return (user?.user_metadata?.full_name || user?.user_metadata?.name || user?.email || '').toString();
  }, [user]);
    ...
    <div className="app-shell">
      ...
      {shouldShowShell && (
        <div className="topbar">
          <div className="brand">
            <div className="logo" aria-hidden="true">DM</div>
            <div>
          ...
          {NAV && NAV.length > 0 && (
            <nav className="nav topnav" aria-label="Primary navigation">
              {NAV.map((n) => (
                ...
                >
                  <span className="nav-icon" aria-hidden="true"><Icon name={n.icon} /></span>
        ...
        <main className="content">
          {shouldShowShell && (pageTitle || pageDescription || pageActions) && (
            <div className="page-header">
              ...
              {pageActions && <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>{pageActions}</div>}
            </div>
          )}
